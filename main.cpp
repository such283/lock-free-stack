...
void test_multiple_producers_consumers() {
    ...
    for (int p = 0; p < num_producers; ++p) {
        producers.emplace_back([&stack, &results, p, items_per_producer]() {
    ...
    for (int c = 0; c < num_consumers; ++c) {
        consumers.emplace_back([&stack, &results, &items_consumed, total_items, c]() {
...
int main() {
    std::cout << "Testing Lock-Free Stack Implementation" << std::endl;
